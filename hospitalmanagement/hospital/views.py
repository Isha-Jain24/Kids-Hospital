...
# for showing signup/login button for admin(by Nikesh)
...
# for showing signup/login button for doctor(by Nikesh)
...
# for showing signup/login button for patient(by Nikesh)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user and user.groups.filter(name='ADMIN').exists():
                ...
                from_email = 'jainisha2499@gmail.com'  # Replace with your email
               ...
               return render(request, 'hospital/adminlogin.html', {'form': form,'error': 'Invalid Email'})  
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user and user.groups.filter(name='DOCTOR').exists():
               ...
               return render(request, 'hospital/doctorlogin.html', {'form': form,'error': 'Invalid Email'})  
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user and user.groups.filter(name='PATIENT').exists():
               ...
               return render(request, 'hospital/patientlogin.html', {'form': form,'error': 'Invalid Email'})  
...
def verify_otp(request):
    ...
    if request.method == 'POST':
        ...
        if user and entered_otp == otp and user.groups.filter(name='ADMIN'):
        ...
        elif user and entered_otp == otp and user.groups.filter(name='DOCTOR').exists():
        ...
        elif user and entered_otp == otp and user.groups.filter(name='PATIENT').exists():
            ...
            return render(request, 'hospital/verify_otp.html', {'error': 'Invalid OTP'})

 

    return render(request, 'hospital/verify_otp.html')

# -----------for checking user is doctor , patient or admin(by Nikesh)
def is_admin(user):
...
# --------------for discharge patient bill (pdf) download and pri
import io
...
def singleblog_view(request):
    return render(request, "hospital/singleblog.html")


def doctordetail_view(request):
    return render(request, "hospital/doctordetail.html")
...
def servicedetail_view(request):
    return render(request, "hospital/servicedetail.html")
